...
add_action('admin_menu', 'broovera_settings_menu');
function broovera_settings_menu() {
	
	//create new top-level menu
	add_menu_page('Broovera Settings', 'Broovera Settings', 'administrator', 'brooverasettingpage', 'broovera_settings_page' , 'dashicons-feedback');
	
	//call register settings function
	add_action( 'admin_init', 'register_broovera_settings' );
}
function register_broovera_settings() {
	//register our settings
	register_setting( 'register_broovera_settings-group', 'cta-box' );
	register_setting( 'register_broovera_settings-group', 'use_logo' );
	register_setting( 'register_broovera_settings-group', 'logo_link' );
	register_setting( 'register_broovera_settings-group', 'desktop_main_menu' );	
}
function broovera_settings_page() {
	?>
<div class="wrap">
<h1>Broovera - Settings Page</h1>

<form method="post" action="options.php">
    <?php settings_fields( 'register_broovera_settings-group' ); ?>
    <?php do_settings_sections( 'register_broovera_settings-group' );
    
    $ctaBox = get_option('cta-box') ;
    $useLogo = get_option('use_logo') ;
    $logoLink = get_option('logo_link') ;
    $desktopmmainMenu = get_option('desktop_main_menu') ;    
    ?>
    ...
    <table class="form-table">
			         ...
			         <tr valign="top">
		<th scope="row">Usare immagine invece del testo come logo</th>
		<td>
		 <?php printf ( '<input value="selected" type="checkbox" id="broovera-use-logo" name="use_logo" %s />', ((isset ( $useLogo) && (!empty($useLogo)) )) ? "checked" : '' ); ?>
		</td>
		</tr>

    	<tr valign="top">
        <th scope="row">Link Immagine Logo</th>
        <td><input type="text" name="logo_link" value="<?php echo esc_attr($logoLink); ?>" /></td>
        </tr>

  
              <tr valign="top">
				<th scope="row">Home Page Testo Principale</th>
				<td>
				  <textarea rows="10" cols="100" name="cta-box"><?php echo $ctaBox;?></textarea>
				</td>
			</tr>
			
		  <tr valign="top">
				<th scope="row">Home Page Testo Secondario</th>
				<td>
				  <textarea rows="10" cols="100" name="desktop_main_menu"><?php echo $desktopmmainMenu;?></textarea>
				</td>
			</tr>				
	</table>    
...
function do_we_want_to_use_logo() {
	return get_option('use_logo', false);
}
